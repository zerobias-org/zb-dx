...
**Who this is for:** You just joined the `zb-poc-devs` effort. You're building a real app against the ZeroBias SDK/client and you've hit something annoying. You want to know what to do with that annoyance.
...
- `--notify` / `-n` — Post an update to `#zb-poc-devs` Slack
...
Claude reads the full entry, uses the ZB MCP to create a feature-request task in the ZB platform, links it in the frontmatter (`zb-task: TASK-4567`), and posts to `#zb-poc-devs`. Status: **task-created**.
...
Claude asks what shipped, records it, and posts to `#zb-poc-devs` so subscribers (you + Dan) get the @-mention. Status: **resolved**.
...
```bash
git clone git@github.com:zerobias-org/zb-poc-devs.git
cd zb-poc-devs
./install.sh
```
...
- **Slack:** `#zb-poc-devs` in the zerobias-org workspace
- **Repo:** [zerobias-org/zb-poc-devs](https://github.com/zerobias-org/zb-poc-devs)
