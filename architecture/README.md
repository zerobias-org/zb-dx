...
- **[`shacl-owl-holon-overlay.html`](./shacl-owl-holon-overlay.html)** — the **ontology overlay**: how the W3C Enterprise stack (OWL 2 + SHACL 1.2 + RDF) maps directly onto ZB's Requirement / Assessment / Record model, and extends into Brian's **Holon → State(T)/Assessment → Hologram** pipeline and the quantum-entangled-task framing. The aim every 3P dev is building toward. Per Brian's 2026-06-05 refinement: a **Holon is a System (system graph / system of systems)**, the **State of System at Time T is the measurement (Assessment)**, and the **Hologram is the projection** of that state onto any surface. Interactive (resizable + fullscreen Mermaid). Standalone hand-authored deliverable (no companion `.md`); deep-dive companion to `transparency-architecture`.
