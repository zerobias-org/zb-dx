...
- **[`transparency-architecture.md`](./transparency-architecture.md)** + **[`transparency-architecture.html`](./transparency-architecture.html)** — the canonical 3P-dev explanation of ZB's **transparency state machine**: Engagement / Project / Board / Task primitives, nested-Project hierarchy, the entangled `satisfies` / `satisfied_by` Task pair, the Transparency Center (HIS ↔ Goshen ↔ ArmorStack multi-org coalition), and the OWL + SHACL + RDF + System / Hologram ontology overlay that culminates in the cross-org compliance-attestation container. Every construct tagged `[EXISTS]` / `[PLANNED]` / `[TBD]`. The `.html` is the interactive version (4 tabs, Mermaid, click-to-filter status, copyable RDF compass + glossary, scroll-spy TOC). The `.html` is **generated from the `.md`** via the visual-explainer skill (dark theme / 16px-floor / mermaid resize+fullscreen prefs) — **regenerate it after editing the `.md`; do not hand-edit the `.html`.**
- **[`shacl-owl-system-overlay.html`](./shacl-owl-system-overlay.html)** — the **ontology overlay**: how the W3C Enterprise stack (OWL 2 + SHACL 1.2 + RDF) maps directly onto ZB's Requirement / Assessment / Record model, and extends into Brian's **System → State(T)/Assessment → Hologram** pipeline and the quantum-entangled-task framing. The aim every 3P dev is building toward. Per Brian's 2026-06-05 refinement: a **System is a System (system graph / system of systems)**, the **State of System at Time T is the measurement (Assessment)**, and the **Hologram is the projection** of that state onto any surface. Interactive (resizable + fullscreen Mermaid). Standalone hand-authored deliverable (no companion `.md`); deep-dive companion to `transparency-architecture`.
